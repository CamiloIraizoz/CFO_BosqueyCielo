...
- **2026-09-16 — Cotizaciones en PDF:** El HTML de `email_sender.py` (tablas anidadas con padding, ancho fijo 620px) revienta en xhtml2pdf con `negative availWidth`. Por eso `execution/pdf_cotizacion.py` tiene plantillas propias: tablas planas con anchos en %, sin anidar, y label/valor como divs en una sola línea (las tablas dentro de una celda se desalinean y los bloques hermanos se reparten verticalmente). **Por qué importa:** si cambias el diseño de la cotización hay DOS plantillas que actualizar — email y PDF — y la de PDF hay que re-renderizarla para verificarla.
